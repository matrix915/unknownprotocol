...
Partial Class UpdateContact
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        ...
        ElseIf ddlMaritalStatus.Text = "Select" Then
            ...
            Dim UpdateContact As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("SqlDataSource1").ConnectionString)
        Dim UpdateCmd As SqlCommand = New SqlCommand("Select * from tblUpdateRegistered WHERE  RelationshipID = '" & ID & "'", UpdateContact)
        Dim UpdateDA As SqlDataAdapter = New SqlDataAdapter()

        UpdateDA.SelectCommand = Cmd
        Connection.Open()
            Dim UpdateDS As DataSet = New DataSet()
        UpdateDA.Fill(UpdateDS, "tblUpdateRegistered")
        Dim UpdateRow As DataRow
        For Each UpdateRow In UpdateDS.Tables("tblUpdateRegistered").Rows

            Update = (UpdateRow("Email").ToString())

        Next

        Connection.Close()

        If Email = Update Then
            ...
            'RegisteredUpdate.UpdateParameters.Add("Email", EmailStatus)
        ...
        Response.Redirect("http://www.unknownprotocol.us:8080/contactinformation.aspx?ID=" & ID)
